...
Tiny Dancer is a high-performance neural routing engine that intelligently routes requests between lightweight and powerful AI models. By scoring candidates in **sub-millisecond latency** (144ns for feature extraction, 7.5µs for model inference), it achieves massive cost savings while maintaining 90-95% quality.

**Key Innovation**: Instead of sending 50-100 memory candidates to expensive LLMs, Tiny Dancer uses a lightweight FastGRNN router (<1MB) to score candidates in microseconds, filtering to only the top 3-5 for expensive evaluation.
